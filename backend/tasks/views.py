...
class TaskDetailView(APIView):
    ...
    def get(self, request, task_id):
        ...
        membership = OrganizationMember.objects.filter(organization=task.organization, user=request.user).first()
        if not membership:
    ...
    def update(self, request, task_id, partial=False):
        ...
        if serializer.is_valid():
            ...
            if updated_task.visibility_type == 'specific':
                creator_membership = OrganizationMember.objects.filter(organization=updated_task.organization, user=updated_task.created_by).first()
    ...
    def delete(self, request, task_id):
        ...
        can_delete = has_permission(request.user, task.organization, 'task_delete_any')
...
class UpdateTaskStatusView(APIView):
    ...
    def patch(self, request, task_id):
        ...
        is_member = OrganizationMember.objects.filter(organization=task.organization, user=request.user).exists()
        if not is_member:
        ...
        if 'assignees' in request.data:
            ...
            if new_assignees is not None:
                ...
                current_ids = set(map(str, task.assignees.values_list('id', flat=True)))
        ...
        if serializer.is_valid():
            ...
            if task.visibility_type == 'specific':
                ...
                creator_membership = OrganizationMember.objects.filter(organization=task.organization, user=task.created_by).first()
                if creator_membership and creator_membership.role == 'user':
                    from users.models import User
                    management_users = User.objects.filter(
...
class TrashView(APIView):
    ...
    def get(self, request, org_id):
        tasks = Task.objects.filter(organization_id=org_id, is_deleted=True).order_by('-updated_at')
        return Response(TaskSerializer(tasks, many=True).data)
...
class TaskAttachmentUploadView(APIView):
    ...
    def post(self, request, task_id):
        ...
        attachment = TaskAttachment.objects.create(task=task, file=file_obj, file_name=file_obj.name, uploaded_by=request.user)
...
class BulkTaskUpdateView(APIView):
    ...
    def post(self, request, org_id):
        ...
        Task.objects.filter(id__in=task_ids, organization_id=org_id).update(**updates)
